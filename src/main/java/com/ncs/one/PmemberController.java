...
public class PmemberController {
	...
	public ModelAndView myboard (ModelAndView mv, SearchCriteria cri, PageMaker pageMaker) {
		// 1) Criteria 처리
		cri.setSnoEno();
	...
	// ** Delete : 회원탈퇴
		...
		public ModelAndView adelete(PmemberVO vo, ModelAndView mv, HttpServletRequest request) {
			// => 삭제 대상 -> vo 에 set
			HttpSession session = request.getSession(false);
			...
			if(service.delete(vo) > 0) {
				...
				mv.addObject("message", "~~ 회원탈퇴 오류 !!  다시 하세요 ~~");
				mv.setViewName("pmember/pCriList");
			}
		}else {
